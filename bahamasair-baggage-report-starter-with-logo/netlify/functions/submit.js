...
exports.handler = async (event, context) => {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      const parsed = parser.parse(event, true); // buffers
      ...
      return {
        statusCode: 400,
        body: `Unsupported Content-Type: ${contentType || "(none)"} — use multipart/form-data or x-www-form-urlencoded.`
      };
    }
    ...
    if (isDebug) {
      return {
        ...
        body: JSON.stringify(
          {
            requestId: (context && (context.awsRequestId || context.invocationId)) || "n/a",
            contentType,
            isBase64: !!event.isBase64Encoded,
            receivedKeys,
            sampleValues: Object.fromEntries(receivedKeys.slice(0, 12).map((k) => [k, (fields[k] || "").slice(0, 120)]))
          },
          null,
          2
        )
    ...
    // honeypot
    if (fields.fax && String(fields.fax).trim() !== "") return { statusCode: 400, body: "Invalid submission" };

    // required
    const required = ["full_name", "email", "phone", "date", "flight", "station", "incident_type", "damage_desc"];
    for (const r of required) {
      if (!fields[r]) {
        return { statusCode: 400, body: `Missing: ${r} | Received keys: ${receivedKeys.join(", ")}` };
      }
    }
    ...
    if (fields.incident_type === "Damaged") {
      for (const r of ["brand_dmg", "age_years", "purchase_price"]) {
        if (!fields[r]) return { statusCode: 400, body: `Missing: ${r} | Received keys: ${receivedKeys.join(", ")}` };
      }
    }
    ...
    const caseId = `BAG-${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}-${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}-${Math.random().toString(36).slice(2, 6).toUpperCase()}`;
    ...
    const html = `
      ...
      <p style="font-size:13px;color:#444;font-family:sans-serif;margin:0;">
        This is an automated message from Bahamasair’s Baggage Reporting System.<br>
        Please reply-all if you need additional information from the passenger.
      </p>
      <br>
      <p style="font-size:13px;color:#0f3a6d;font-family:sans-serif;margin:0;font-weight:600;">
    ...
    if (fields.email) msg.cc = String(fields.email);
